<footer class="footer spad">
    <div class="container">
        <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-6">
                <div class="footer__about">
                    <div class="footer__about__logo">
                        <a href="./index.html"><img src="{{url("ogani/img/logo2.png")}}" style="width:120px; height:66px" alt=""></a>
                    </div>
                    <ul>
                        <li>Alamat: </li>
                        <li>Email: dukuhkerupuk@.com</li>
                    </ul>
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-sm-6 offset-lg-1">
                <div class="footer__widget">
                    <h6>Jelajahi Situs</h6>
                    <ul>
                        <li><a href="#">Tentang Kami</a></li>
                        <li><a href="#">Keamanan Transaksi</a></li>
                        <li><a href="#">Informasi Pengiriman</a></li>
                        <li><a href="#">Kebijakan Kami</a></li>
                        <li><a href="#">Blog</a></li>
                    </ul>
                    <ul>
                        <li><a href="#">Bantuan</a></li>
                        <li><a href="#">Hubungi Kami</a></li>
                        <li><a href="#">Metode Pembayaran</a></li>
                        <li><a href="#">Ongkos Kirim</a></li>
                        <li><a href="#">Testimoni</a></li>
                    </ul>
                </div>
            </div>
            <div class="col-lg-4 col-md-12">
                <div class="footer__widget">
                    <h6>Gabung Newsletter Kami Sekarang</h6>
                    <p>Dapatkan email tentang promo terbaru kami.</p>
                    <form action="POST" action="{{route('subscriber.post')}}">
                        @csrf
                        <input type="text" name="From" placeholder="Nomor Telepon">
                        <button type="submit" class="site-btn">Gabung</button>
                    </form>
                    <div class="footer__widget__social">
                        <a href="#"><i class="fa fa-facebook"></i></a>
                        <a href="#"><i class="fa fa-instagram"></i></a>
                        <a href="#"><i class="fa fa-twitter"></i></a>
                        <a href="#"><i class="fa fa-pinterest"></i></a>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="footer__copyright">
                    <div class="footer__copyright__text"><p><!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
Copyright &copy;<script>document.write(new Date().getFullYear());</script> All rights reserved Dukuhkerupuk</a>
<!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. --></p></div>
                    <div class="footer__copyright__payment"><img src="img/payment-item.png" alt=""></div>
                </div>
            </div>
        </div>
    </div>
</footer>